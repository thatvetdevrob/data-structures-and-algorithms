...
const templateWithJQuery = () => {
  ...
  // Const might be messing me up

  let newCharacter = $('#template').html();

  starWarsPeople.forEach(hood => {
    // The Sith Would consider Skywalkers some criminal hood....

    let iAmInStarWars = $(`<section>${newCharacter}</section>`);

    iAmInStarWars.find('h2').text(hood.name);

    iAmInStarWars.find('h3').text(hood.height);

    iAmInStarWars.find('p').text(hood.eye_color);

    $('main').append(iAmInStarWars);

    // $( '#template > h2:nth-child(1)').after(`<h2>${hood.name}</h2>`);

    // $( '#template > h3').after(`<h3>${hood.height}</h3>`);

    // $( '#template > p').after(`<p>${hood.eye_color}</p>`);

    // $( '#template' ).append(`<h3>${hood.height}</h3>`);

    // $( '#template' ).append(`<p>${hood.eye_color}</p>`);

    // $('#template').prepend(`<h2>${hood.name}</h2>`);

    // $('#template').prepend(`<h3>${hood.height}</h3>`);

    // $('#template').prepend(`<p>${hood.eye_color}</p>`);

    // let arrayOfHtml = $.html();

    // console.log('This is new s? ', arrayOfHtml);

    // console.log('This is it: ', $('section:nth-child(2) h2').text());

    // return arrayOfHtml;

    // Console.log says everything is there, but i cant seem to be able to return it?
...
const listFoods = (recipe) => {
  ...
  //DONT USE SPLITTTTTTTTT..... and on that note.....

  //   Keith's Banana Split by Susie Oviatt
  ...
  recipe.ingredients.forEach(grufalo => {
    let arr = [];

    // i need to find the index of the empty space right after the word i *dont need*
    let x = grufalo.indexOf(' ');

    while (x !== -1) {
      arr.push(x);
      x = grufalo.indexOf(' ', x + 1);
    }

    result.push(grufalo.slice(arr[1] + 1, grufalo.length));
  });
  return result;
