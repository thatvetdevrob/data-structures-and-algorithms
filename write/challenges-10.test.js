...
const createServer = () => {
  ...
  app.get('/aboutme', (req, resp) => {
    let bio = 'bio bio bio';
    resp.status(200).send(bio);
  });

  app.get('/favoritefoods', (req, resp) => {
    let food = ['Mad Pizza', 'Pho', 'Jack in The Box'];
    resp.status(200).send(food);
  });

  // app.get('*', (req, resp) => {
  //   resp.status(404).send('Sorry');
  // });
...
const totalSum = (input) => {
  ...
  return input.reduce((a, b) => {
    //may need to use the same method more than once
    return a + b.reduce((sum, next) => {
      // here we add them all together for the total
      sum = sum + next;
      return sum;
    }, 0);
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  // modulous takes care of 5, and i need to make sure its not anything but a number
    return arr.reduce((hungryArr, vari) => {
      if(vari%5 ===0 && typeof(vari)==='number'){
        // giving food to the array: sorry for the lack of art, im tired.
        hungryArr.push(Math.pow(2, vari));
        return hungryArr;
      }
      else {
        return hungryArr;
    ...
    // the empty array there up one line is what i would image means we are returining? i need to re read but sounds right?
